...
# [] # 使用时放开
nodes = asyncio.run(jina.split_text_with_jina(short_psg, max_chunk_length))
print(nodes)
print(f"JINA分割初步切分成 {len(nodes)} 个chunk")
print("---------------------------")
# TODO 确认 合并后的顺序问题， 集成到系统里
# 合并短句，确保语义完整性 # TODO 目标 4合2
big_chunks = []
current_text = []
current_length = 0
...
for i, node in enumerate(nodes):
    # 如果当前句子很短，尝试与下一句合并 # TODO 寻找合适的参数 100
    if len(node) < 20 and len(node.split()) < 2 and current_length < max_chunk_length:
        current_text.append(node)
        current_length += len(node.split())
        small_big_dict.update({i: big_chunk_id}) # NEW
    else: # 如果当前句子不是很短，则
        # 如果积累了一些短句，将它们合并
        if current_text: # 如果积累了一些短句，将它们合并
...
for i, node in enumerate(nodes):
    ...
    vc_nodes.append({
        "metadata": {"article_id": article_id, "article_title": article_title, 
            "chunk_id": f"chunk_{chunk_id}", "article_chunk_id": f"article_{article_id}_chunk_{chunk_id}"},
        "pk": i,
        "content": node,
        "embedding": doc_embeddings[i] # print dimension
    })
# client.insert(
#     collection_name="my_json_collection",
#     data=vc_nodes
...
# 将父文档缓存
# TODO 如何为每个chunk绑定child ids
# TODO redis_client.set_string("90", "chunk_19")
# redis map current: {article1_chunks: {chunk_19: "巨龙的黎明时间线..."}}
# redis map: {article_chunks: {article_1_chunk_19: "..."}}
# for i, big_chunk in enumerate(big_chunks):
#     # give big chunk a unique id
#     chunk_id = f"chunk_{i}"
#     # save big chunk to redis
#     redis_client.set_hash(f"article_chunks", {chunk_id: big_chunk})
print(" ---------------- ")

# # chunk_id = redis_client.get_string("90")
# # print(chunk_id)
# # get demo
# # "metadata": {"article_id": article_1, "chunk_id": "chunk_19", "article_chunk_id": article_1_chunk_19}
# filter='metadata["article_title"] == "魔兽时间线"'
# children = client.search(
#     collection_name="my_json_collection",
#     data=[voyager.get_query_embedding("巨龙的黎明时间线？")],
#     limit=10,
#     search_params={"params": {"nprobe": 10}},
...
# children = children[0]
# print(children)
# # {'id': 90, 'distance': 0.6731717586517334, 'entity': {'content': '（五）当期处置的投资性房地产及其对损益的影响。\n', 'metadata': {...}}}
# chunk_ids = []
# for i, node in enumerate(children):
#     chunk_id = node.get("entity").get("metadata").get("article_chunk_id")
